...
expression.raw.samples.dglist <- expression.raw.samples.dglist[rowSums(cpm(expression.raw.samples.dglist) > 5) >= 1,]
...
par(mar = c(6,6,5,3))
group1<-groups[1]
group_colors<-rep("red",length(groups))
for(i in 1:length(groups)){if(groups[i] != group1){group_colors[i]<-"blue"}}
plotMDS(expression.raw.samples.dglist, main=paste("MDS plot for ",label,sep=""), col=group_colors)
#dev.off()
